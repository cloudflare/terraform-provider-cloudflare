...
func TestAccCloudflareSplitTunnelExclude(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCloudflareSplitTunnelExclude(rnd, accountID, "example domain", "*.example.com"),
...
func testAccCloudflareSplitTunnelExclude(rnd, accountID string, description string, host string) string {
	return fmt.Sprintf(`
...
func testAccCheckCloudflareSplitTunnelExcludeDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.SplitTunnelExclude(context.Background(), rs.Primary.Attributes["account_id"])
		if err == nil {
