...
import (
	...
	"github.com/cloudflare/terraform-provider-cloudflare/cmd/migrate-v2/transformations/config"
)
...
// NewBaseMigration creates a new base migration from YAML configuration
func NewBaseMigration(yamlContent []byte) (*BaseMigration, error) {
...
func (m *BaseMigration) ResourceType() string {
...
func (m *BaseMigration) MigrateConfig(block *hclwrite.Block, ctx *MigrationContext) error {
	...
	if len(m.config.Config.AttributeRemovals) > 0 {
		remover := common.AttributeRemover(m.config.Config.AttributeRemovals...)
	...
	for _, removal := range m.config.Config.ConditionalRemovals {
		...
		condRemover := common.ConditionalRemover(removal.Attribute, condition)
	...
	for _, blockType := range m.config.Config.BlocksToLists {
		converter := config.BlocksToListConverter(blockType)
	...
	for _, attrName := range m.config.Config.ListsToBlocks {
		converter := config.ListToBlocksConverter(attrName)
		if err := converter(block, transformCtx); err != nil {
	...
	if len(m.config.Config.DefaultValues) > 0 {
		setter := common.DefaultValueSetter(m.config.Config.DefaultValues)
	...
	for _, conversion := range m.config.Config.TypeConversions {
		if conversion.FromType == "set" && conversion.ToType == "list" {
			...
			converter := common.SetToListConverter(conversion.Attribute)
			if err := converter(block, transformCtx); err != nil {
...
func (m *BaseMigration) MigrateState(state map[string]interface{}, ctx *MigrationContext) error {
...
func (m *BaseMigration) Validate(block *hclwrite.Block) error {
	// Basic validation - check if resource type matches
	return nil
}

// createConditionFunc creates a condition function for ConditionalRemover
func (m *BaseMigration) createConditionFunc(removal ConditionalRemoval) func(*hclwrite.Block) bool {
...
func (m *BaseMigration) applyTypeConversion(block *hclwrite.Block, conversion TypeConversion, ctx *MigrationContext) {
	body := block.Body()
...
func (m *BaseMigration) applyStateTypeConversion(state map[string]interface{}, conversion TypeConversion, ctx *MigrationContext) {
...
func (m *BaseMigration) applyStructuralChange(block *hclwrite.Block, change StructuralChange, ctx *MigrationContext) error {
...
func (m *BaseMigration) applySplitObject(block *hclwrite.Block, change StructuralChange, ctx *MigrationContext) error {
	transformCtx := &common.TransformContext{}
	...
	if attrMap, ok := change.Parameters["attribute_map"].(map[string]interface{}); ok {
		...
		transformer := config.SplitObjectWithMapping(change.Source, mappedAttrs)
	...
	transformer := config.SplitObjectTransformer(change.Source, attributes, prefix)
	return transformer(block, transformCtx)
}

// applyMergeAttributes merges multiple attributes into one
func (m *BaseMigration) applyMergeAttributes(block *hclwrite.Block, change StructuralChange, ctx *MigrationContext) error {
	transformCtx := &common.TransformContext{}
	...
	if attrMap, ok := change.Parameters["attribute_map"].(map[string]interface{}); ok {
		...
		transformer := config.MergeAttributesWithMapping(change.Target, mappedAttrs)
		return transformer(block, transformCtx)
	}
	...
	transformer := config.MergeAttributesTransformer(change.Target, sourceAttrs, format)
	return transformer(block, transformCtx)
}

// applyNestedRestructure restructures nested attributes
func (m *BaseMigration) applyNestedRestructure(block *hclwrite.Block, change StructuralChange, ctx *MigrationContext) error {
	transformCtx := &common.TransformContext{}
	...
	if paths, ok := change.Parameters["paths"].(map[string]interface{}); ok {
		...
		transformer := config.NestedRestructureTransformer(change.Source, pathMap)
		return transformer(block, transformCtx)
	}
	...
	if prefix, ok := change.Parameters["prefix"].(string); ok && prefix != "" {
		transformer := config.FlattenWithPrefix(change.Source, prefix, separator)
		return transformer(block, transformCtx)
	}
	
	// Use regular flatten
	transformer := config.FlattenNestedTransformer(change.Source, separator, maxDepth)
	return transformer(block, transformCtx)
}

// applyTransform applies transformation-based structural changes
func (m *BaseMigration) applyTransform(block *hclwrite.Block, change StructuralChange, ctx *MigrationContext) error {
	transformCtx := &common.TransformContext{}
	...
	case "string_list_to_object_list":
		...
		if updateRefs, ok := change.Parameters["update_references"].([]interface{}); ok && len(updateRefs) > 0 {
			if ref, ok := updateRefs[0].(map[string]interface{}); ok {
				...
				if from != "" && to != "" {
					...
					transformer := config.StringArrayToObjectArrayWithRename(change.Source, objectKey, from, to)
					return transformer(block, transformCtx)
				}
			}
		}
		...
		transformer := config.ArrayToObjectArrayConverter(change.Source, objectKey)
		return transformer(block, transformCtx)

	case "object_to_list":
		// Convert nested blocks to a list attribute
		converter := config.BlocksToListConverter(change.Source)
	...
	case "list_to_blocks":
		...
		if attrMap, ok := change.Parameters["attribute_map"].(map[string]interface{}); ok {
			...
			transformer := config.ListToBlocksWithMapping(change.Source, mappedAttrs)
			return transformer(block, transformCtx)
		}
		
		// Use simple converter
		transformer := config.ListToBlocksConverter(change.Source)
		return transformer(block, transformCtx)
