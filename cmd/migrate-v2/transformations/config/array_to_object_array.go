...
func ArrayToObjectArrayConverter(attributeName string, wrapperField string) common.TransformerFunc {
...
func StringArrayToObjectArrayWithRename(attributeName string, wrapperField string, oldPrefix string, newPrefix string) common.TransformerFunc {
	return func(block *hclwrite.Block, ctx *common.TransformContext) error {
		body := block.Body()
		attr := body.GetAttribute(attributeName)
