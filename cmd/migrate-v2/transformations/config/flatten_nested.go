...
func FlattenNestedTransformer(source string, separator string, maxDepth int) common.TransformerFunc {
...
func FlattenWithPrefix(source string, prefix string, separator string) common.TransformerFunc {
	if separator == "" {
		separator = "_"
	}
...
func NestedRestructureTransformer(source string, paths map[string]string) common.TransformerFunc {
	return func(block *hclwrite.Block, ctx *common.TransformContext) error {
		body := block.Body()
		attr := body.GetAttribute(source)
