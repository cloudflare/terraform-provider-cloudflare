...
// The v4 state has schema_version=0 (SDKv2 default), and we transform it to v5 format with new computed fields.
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading regional_tiered_cache state from v4 SDKv2 provider (schema_version=0)")

	// Parse v4 state using v4 model
	var v4State SourceCloudflareRegionalTieredCacheModel
...
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading regional_tiered_cache state from version=1 to version=500 (no-op)")
