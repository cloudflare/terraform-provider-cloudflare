...
func testSweepCloudflarePageRules(r string) error {
	...
	if err != nil {
		tflog.Error(ctx, fmt.Sprintf("Failed to fetch page rules for zone %s: %s", zoneID,err))
		return err
	}
	if len(pageRules) == 0 {
	...
	if err != nil {
		tflog.Error(ctx, fmt.Sprintf("Failed to fetch page rules for alt zone %s: %s", altZoneID,err))
		return err
	}
