...
// - v4 state: actions is an ARRAY (ListNestedAttribute)
// - v5.16.0 state: actions is an OBJECT (SingleNestedAttribute)
//
// Detection strategy: Parse raw JSON and check if actions is array (v4) or object (v5).
func UpgradeFromVersion0(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading page_rule state from version=0 (detecting v4 vs v5.16.0 format)")
...
func unmarshalV4State(ctx context.Context, rawState *tfprotov6.RawState, target *SourceV4PageRuleModel) diag.Diagnostics {
	var diags diag.Diagnostics

	sourceSchema := SourceV4PageRuleSchema()
