...
func TestAccCloudflareZeroTrustConnectivityDirectoryServiceDataSource_Basic(t *testing.T) {
	rnd := utils.GenerateRandomResourceName()
	accountID := os.Getenv("CLOUDFLARE_ACCOUNT_ID")
	resourceName := "cloudflare_zero_trust_connectivity_directory_service." + rnd
	dataSourceName := "data.cloudflare_zero_trust_connectivity_directory_service." + rnd
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCloudflareZeroTrustConnectivityDirectoryServiceDataSource_Basic(
					rnd,
					accountID,
					serviceName,
					hostname,
...
func testAccCloudflareZeroTrustConnectivityDirectoryServiceDataSource_Basic(rnd, accountID, serviceName, hostname, resolverIP1, resolverIP2 string, httpPort, httpsPort int) string {
