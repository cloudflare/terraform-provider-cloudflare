...
var _ resource.ResourceWithModifyPlan = (*SecretsStoreResource)(nil)
...
// SecretsStoreResource defines the resource implementation.
...
func (r *SecretsStoreResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	res := new(http.Response)
	env := SecretsStoreResultEnvelope{*data}
	_, err := r.client.SecretsStore.Stores.Get(
		ctx,
		data.ID.ValueString(),
		secrets_store.StoreGetParams{
			AccountID: cloudflare.F(data.AccountID.ValueString()),
		},
		option.WithResponseBodyInto(&res),
		option.WithMiddleware(logging.Middleware(ctx)),
...
func (r *SecretsStoreResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	data.ID = types.StringValue(path_store_id)
...
func (r *SecretsStoreResource) ModifyPlan(_ context.Context, _ resource.ModifyPlanRequest, _ *resource.ModifyPlanResponse) {
