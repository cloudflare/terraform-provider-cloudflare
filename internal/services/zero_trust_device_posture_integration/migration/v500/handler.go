...
// PriorSchema is nil because v4 and v5 have incompatible schemas for config:
// - v4 state: config is an ARRAY (ListNestedAttribute, needs transformation)
// - v5.16.0 state: config is an OBJECT (SingleNestedAttribute, no-op)
//
// Detection strategy: Parse raw JSON and check if config is array or object.
func UpgradeFromVersion0(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_device_posture_integration state from version=0 (detecting v4 vs v5.16.0 format)")
	...
	if isV4 {
		tflog.Info(ctx, "Detected v4 state format (config is array), performing transformation")
		upgradeFromV4Internal(ctx, req, resp)
	} else {
		tflog.Info(ctx, "Detected v5.16.0+ state format (config is object), no-op upgrade")
		unmarshalV5StateToResponse(ctx, req.RawState, resp)
	}
}

// detectV4State checks if the state is v4 format by inspecting config field.
// v4: config is an array ([]interface{})
// v5: config is an object (map[string]interface{})
	...
	if req.RawState != nil && len(req.RawState.JSON) > 0 {
		...
		if err := json.Unmarshal(req.RawState.JSON, &rawJSON); err == nil {
			if configRaw, ok := rawJSON["config"]; ok {
				...
				case []interface{}:
					return true, nil // v4 format: config is an array
				case map[string]interface{}:
					return false, nil // v5 format: config is an object
				}
			}
			// No config field - treat as v5 (config might be null)
			return false, nil
		}
	}
...
func upgradeFromV4Internal(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_device_posture_integration state from v4 SDKv2 provider (schema_version=0)")

	// Parse v4 state using v4 schema and model
	var v4State SourceCloudflareDevicePostureIntegrationModel
...
func unmarshalV4State(ctx context.Context, rawState *tfprotov6.RawState, target *SourceCloudflareDevicePostureIntegrationModel) diag.Diagnostics {
	var diags diag.Diagnostics

	sourceSchema := SourceCloudflareDevicePostureIntegrationSchema()
...
//
// Version 1 is the "dormant" state for v5 resources before migration activation.
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_device_posture_integration state from version=1 to version=500 (no-op)")
