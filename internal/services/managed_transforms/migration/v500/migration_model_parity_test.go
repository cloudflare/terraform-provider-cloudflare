...
import (
	...
	v500 "github.com/cloudflare/terraform-provider-cloudflare/internal/services/managed_transforms/migration/v500"
	"github.com/cloudflare/terraform-provider-cloudflare/internal/test_helpers"
)

// TestManagedTransformsMigrationModelSchemaParity verifies that TargetManagedTransformsModel
// (used in UpgradeFromV0 → resp.State.Set, MoveState → resp.TargetState.Set) stays in sync with the live ResourceSchema.
func TestManagedTransformsMigrationModelSchemaParity(t *testing.T) {
	t.Parallel()
	model := (*v500.TargetManagedTransformsModel)(nil)
