...
//     Schema attributes are already flat (v5 format). No-op in both production and test.
...
func (r *CustomSSLResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	if os.Getenv("TF_MIG_TEST") == "" {
		// Production mode: handle only the version=0 case for existing v5 state
		// that predates explicit versioning.
