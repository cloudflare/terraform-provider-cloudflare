...
type APIShieldOperationDiscoveryResultListDataSourceEnvelope struct {
	Result customfield.NestedObjectList[APIShieldOperationDiscoveryDataSourceModel] `json:"result,computed"`
}

type APIShieldOperationDiscoveryDataSourceModel struct {
	...
	Features    customfield.NestedObject[APIShieldOperationDiscoveryFeaturesDataSourceModel] `tfsdk:"features" json:"features,computed"`
	Filter      *APIShieldOperationDiscoveryFindOneByDataSourceModel                         `tfsdk:"filter"`
}

func (m *APIShieldOperationDiscoveryDataSourceModel) toListParams(_ context.Context) (params api_gateway.DiscoveryOperationListParams, diags diag.Diagnostics) {
	mFilterHost := []string{}
...
type APIShieldOperationDiscoveryFeaturesDataSourceModel struct {
	TrafficStats customfield.NestedObject[APIShieldOperationDiscoveryFeaturesTrafficStatsDataSourceModel] `tfsdk:"traffic_stats" json:"traffic_stats,computed"`
}

type APIShieldOperationDiscoveryFeaturesTrafficStatsDataSourceModel struct {
	LastUpdated   timetypes.RFC3339 `tfsdk:"last_updated" json:"last_updated,computed" format:"date-time"`
...
type APIShieldOperationDiscoveryFindOneByDataSourceModel struct {
