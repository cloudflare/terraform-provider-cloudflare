...
// TestUnmarshalCustom exercises the public UnmarshalCustom entry point with
// real envelope JSON, verifying that apijson deserialization and domain
// reordering work together end-to-end.
func TestUnmarshalCustom(t *testing.T) {
	// Simulate an API response with domains in alphabetical order.
	apiResponse := map[string]interface{}{
		"result": map[string]interface{}{
			"sitekey":    "0x4AAAAAAA",
			"name":       "test-widget",
			"mode":       "managed",
			"domains":    []string{"alpha.example.com", "middle.example.com", "zebra.example.com"},
			"secret":     "0x4AAAAAAA_secret",
	...
	// Pre-populate the envelope with the user's planned order.
	env := TurnstileWidgetResultEnvelope{}
	env.Result.Domains = domainSlice("zebra.example.com", "alpha.example.com", "middle.example.com")

	err = UnmarshalCustom(data, &env)
	if err != nil {
		t.Fatalf("UnmarshalCustom returned error: %v", err)
	}

	// Domains must be reordered to match the planned order.
	want := []string{"zebra.example.com", "alpha.example.com", "middle.example.com"}
	if got := domainStrings(env.Result.Domains); !stringsEqual(got, want) {
		t.Fatalf("UnmarshalCustom domains = %v, want %v", got, want)
	}

	// Other fields must be populated from the JSON.
	if got := env.Result.Name.ValueString(); got != "test-widget" {
		t.Fatalf("UnmarshalCustom name = %q, want %q", got, "test-widget")
	}
}

// TestUnmarshalComputedCustom exercises the public UnmarshalComputedCustom
// entry point, which is used on Create/Update where computed fields are
...
func TestUnmarshalComputedCustom(t *testing.T) {
	apiResponse := map[string]interface{}{
		"result": map[string]interface{}{
			...
			"name":       "created-widget",
			"mode":       "invisible",
			"domains":    []string{"alpha.example.com", "zebra.example.com"},
			"secret":     "0x4BBBBBB_secret",
	...
	env := TurnstileWidgetResultEnvelope{}
	env.Result.Domains = domainSlice("zebra.example.com", "alpha.example.com")

	err = UnmarshalComputedCustom(data, &env)
	if err != nil {
		t.Fatalf("UnmarshalComputedCustom returned error: %v", err)
	}

	want := []string{"zebra.example.com", "alpha.example.com"}
	if got := domainStrings(env.Result.Domains); !stringsEqual(got, want) {
		t.Fatalf("UnmarshalComputedCustom domains = %v, want %v", got, want)
	}

	// Computed field must be populated.
	if got := env.Result.Sitekey.ValueString(); got != "0x4BBBBBB" {
		t.Fatalf("UnmarshalComputedCustom sitekey = %q, want %q", got, "0x4BBBBBB")
	}
}
