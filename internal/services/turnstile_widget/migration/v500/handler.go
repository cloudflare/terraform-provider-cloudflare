...
// UpgradeFromV4 handles state upgrades from v4 Plugin Framework provider (schema_version=0) to v5 (version=500).
//
...
//
// Key transformations:
// - domains: types.Set → *[]types.String (with alphabetical sorting)
// - All other fields: direct pass-through
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading turnstile_widget state from v4 provider (schema_version=0)")

	// Parse v4 state using v4 model
	var v4State SourceCloudfareTurnstileWidgetModel
...
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading turnstile_widget state from version=1 to version=500 (no-op)")
