...
func (m *ZeroTrustDNSLocationsDataSourceModel) toListParams(_ context.Context) (params zero_trust.GatewayLocationListParams, diags diag.Diagnostics) {
	...
	if !m.Direction.IsNull() {
		params.Direction = cloudflare.F(zero_trust.GatewayLocationListParamsDirection(m.Direction.ValueString()))
	}
	if !m.OrderBy.IsNull() {
		params.OrderBy = cloudflare.F(zero_trust.GatewayLocationListParamsOrderBy(m.OrderBy.ValueString()))
	}
	if !m.Search.IsNull() {
...
type ZeroTrustDNSLocationsResultDataSourceModel struct {
	...
	MaxTTL                    customfield.NestedObject[ZeroTrustDNSLocationsMaxTTLDataSourceModel]       `tfsdk:"max_ttl" json:"max_ttl,computed"`
...
type ZeroTrustDNSLocationsMaxTTLDataSourceModel struct {
