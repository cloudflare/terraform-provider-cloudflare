...
// PriorSchema is nil because v4 and v5 have incompatible schemas for context_awareness:
// - v4 state: context_awareness is an ARRAY (ListNestedAttribute, needs transformation)
// - v5.16.0 state: context_awareness is an OBJECT (SingleNestedAttribute, no-op)
//
// Detection strategy: Parse raw JSON and check if context_awareness is array or object.
func UpgradeFromVersion0(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_dlp_custom_profile state from version=0 (detecting v4 vs v5.16.0 format)")
	...
	if isV4 {
		tflog.Info(ctx, "Detected v4 state format (context_awareness is array), performing transformation")
		upgradeFromV4Internal(ctx, req, resp)
	} else {
		tflog.Info(ctx, "Detected v5.16.0+ state format (context_awareness is object), no-op upgrade")
		unmarshalV5StateToResponse(ctx, req.RawState, resp)
	}
}

// detectV4State checks if the state is v4 format by inspecting context_awareness field.
// v4: context_awareness is an array ([]interface{})
// v5: context_awareness is an object (map[string]interface{}) or null
func detectV4State(req resource.UpgradeStateRequest) (bool, error) {
	if req.RawState != nil && len(req.RawState.JSON) > 0 {
		...
		if err := json.Unmarshal(req.RawState.JSON, &rawJSON); err == nil {
			if caRaw, ok := rawJSON["context_awareness"]; ok && caRaw != nil {
				switch caRaw.(type) {
				case []interface{}:
					return true, nil // v4 format: context_awareness is an array
				case map[string]interface{}:
					return false, nil // v5 format: context_awareness is an object
				}
			}
			// No context_awareness field or null - treat as v5
			return false, nil
		}
	}
...
func upgradeFromV4Internal(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_dlp_custom_profile state from v4 SDKv2 provider (schema_version=0)")

	var v4State SourceCloudflareDLPProfileModel
	diags := unmarshalV4State(ctx, req.RawState, &v4State)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		tflog.Error(ctx, "Failed to parse v4 state for zero_trust_dlp_custom_profile",
	...
	tflog.Info(ctx, "State upgrade from v4 to v5 completed successfully for zero_trust_dlp_custom_profile")
}
...
func UpgradeFromV1(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading zero_trust_dlp_custom_profile state from version=1 to version=500 (no-op)")

	resp.State.Raw = req.State.Raw

	tflog.Info(ctx, "State version bump from 1 to 500 completed for zero_trust_dlp_custom_profile")
}
