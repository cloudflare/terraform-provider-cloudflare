...
// UpgradeFromV4 handles state upgrades from schema_version=0 to version=500.
//
// This handles TWO scenarios:
// 1. v4 SDKv2 state (has "type" field) - needs transformation
// 2. Early v5 state (has "identifier" field but version=0) - just needs version bump
//
// We detect which format by attempting to parse as v5 first. If identifier is present
...
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading custom_pages state from schema_version=0")

	// Try to parse as v5 state first (PriorSchema is target schema)
	var v5State TargetCustomPagesModel
	...
	tflog.Debug(ctx, "Attempted to parse as v5 state", map[string]interface{}{
		...
		"identifier_is_null":   v5State.Identifier.IsNull(),
		"identifier_unknown":   v5State.Identifier.IsUnknown(),
		"identifier_value":     v5State.Identifier.ValueString(),
	})

	// Check if this is valid v5 state by seeing if identifier was populated
	if !diags.HasError() && !v5State.Identifier.IsNull() && !v5State.Identifier.IsUnknown() {
		// This is early v5 state with version=0 - just bump the version
		tflog.Info(ctx, "Detected early v5 state (has identifier), performing no-op version bump")
		resp.State.Raw = req.State.Raw
		return
	}

	// This is v4 state - we need to parse it from raw JSON since PriorSchema won't match
	tflog.Info(ctx, "Detected v4 state (missing identifier), performing transformation from raw state")

	// Parse v4 state using the v4 schema
	var v4State SourceCloudflareCustomPagesModel
...
func unmarshalV4State(ctx context.Context, rawState *tfprotov6.RawState, target *SourceCloudflareCustomPagesModel) diag.Diagnostics {
	var diags diag.Diagnostics

	// Get the v4 schema
	sourceSchema := SourceCloudflareCustomPagesSchema()
...
// No transformation is needed because v5 schema hasn't changed, just the version number.
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading custom_pages state from version=1 to version=500 (no-op)")
