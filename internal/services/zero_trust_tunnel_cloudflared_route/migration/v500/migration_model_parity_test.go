...
// TestZeroTrustTunnelCloudflaredRouteMigrationModelSchemaParity verifies that TargetTunnelRouteModel
// (used in UpgradeFromV4 → resp.State.Set, MoveState → resp.TargetState.Set) stays in sync with the live ResourceSchema.
func TestZeroTrustTunnelCloudflaredRouteMigrationModelSchemaParity(t *testing.T) {
	t.Parallel()
	model := (*v500.TargetTunnelRouteModel)(nil)
