...
func (m *AccountDNSSettingsInternalViewsDataSourceModel) toListParams(_ context.Context) (params dns.SettingZoneViewListParams, diags diag.Diagnostics) {
	params = dns.SettingZoneViewListParams{
		AccountID: cloudflare.F(m.AccountID.ValueString()),
	}

	if !m.Direction.IsNull() {
		params.Direction = cloudflare.F(dns.SettingZoneViewListParamsDirection(m.Direction.ValueString()))
	}
	if !m.Match.IsNull() {
		params.Match = cloudflare.F(dns.SettingZoneViewListParamsMatch(m.Match.ValueString()))
	}
	if m.Name != nil {
	...
	if !m.Order.IsNull() {
		params.Order = cloudflare.F(dns.SettingZoneViewListParamsOrder(m.Order.ValueString()))
	}
	if !m.ZoneID.IsNull() {
