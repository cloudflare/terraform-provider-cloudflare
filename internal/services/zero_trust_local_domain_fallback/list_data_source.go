...
var _ datasource.DataSourceWithConfigure = (*ZeroTrustLocalDomainFallbacksDataSource)(nil)
...
func (d *ZeroTrustLocalDomainFallbacksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	items := customfield.NullObjectList[ZeroTrustLocalDomainFallbacksResultDataSourceModel](ctx)
	...
	acc = acc[:maxItems]
	result, diags := customfield.NewObjectListFromAttributes[ZeroTrustLocalDomainFallbacksResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
