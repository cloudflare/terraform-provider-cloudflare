...
// TestHyperdriveConfigMigrationModelSchemaParity verifies that TargetHyperdriveConfigModel
// (used in UpgradeFromV0 -> resp.State.Set) stays in sync with the live ResourceSchema.
func TestHyperdriveConfigMigrationModelSchemaParity(t *testing.T) {
	t.Parallel()
	model := (*v500.TargetHyperdriveConfigModel)(nil)
