...
import (
	...
	v500 "github.com/cloudflare/terraform-provider-cloudflare/internal/services/workers_route/migration/v500"
	"github.com/cloudflare/terraform-provider-cloudflare/internal/test_helpers"
)

// TestWorkersRouteMigrationModelSchemaParity verifies that TargetWorkersRouteModel
// (used in UpgradeFromV0 → resp.State.Set, MoveState → resp.TargetState.Set) stays in sync with the live ResourceSchema.
func TestWorkersRouteMigrationModelSchemaParity(t *testing.T) {
	t.Parallel()
	model := (*v500.TargetWorkersRouteModel)(nil)
