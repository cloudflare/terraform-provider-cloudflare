...
// - Passes through account_id, zone_id, mode, notes
// - Initializes new computed fields (id, created_on, modified_on, allowed_modes, scope)
//
// The v4 state has schema_version=1 (after v0→v1 migration in SDKv2 provider).
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading access_rule state from v4 SDKv2 provider (schema_version=1)")
...
//
// Version 1 is the "dormant" v5 state before migration activation.
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading access_rule state from version=1 to version=500 (no-op)")
