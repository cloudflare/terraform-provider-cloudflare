...
func TestComputeItemsHash(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// Regression: with a naive "\x00" intra-item separator, {value="a", desc="b"}
			// and {value="a\x00b", desc=""} both encode as "a\x00b", producing a false match.
...
// TestComputeItemsHashDeterministic verifies the hash is deterministic (same
...
// TestComputeItemsHashLargeList verifies the hash completes on large lists.
