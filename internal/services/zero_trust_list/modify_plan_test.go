...
// TestModifyPlan_NullPlanReturnsEarly verifies that ModifyPlan is a no-op on
// destroy (null plan), consistent with the req.Plan.Raw.IsNull() guard.
...
// TestModifyPlan_NullStateReturnsEarly verifies that ModifyPlan is a no-op on
// first create (null state), consistent with the req.State.Raw.IsNull() guard.
...
// TestModifyPlan_SameItemsSuppressesDiff verifies that identical plan and
// state items result in the plan being normalized to state (no-op diff
// suppression).
...
// TestModifyPlan_DifferentItemsDoesNotSuppressDiff verifies that when plan
// and state items differ, ModifyPlan does NOT overwrite the plan, the diff
// is preserved.
...
// TestModifyPlan_NilPlanItemsSkipsComparison verifies that when plan has no
// items (nil), ModifyPlan does not attempt a hash comparison or panic.
...
// TestModifyPlan_NilStateItemsSkipsComparison verifies that when state has no
// items (nil), ModifyPlan does not attempt a hash comparison or panic.
...
// TestModifyPlan_ReorderedItemsSuppressesDiff verifies that items in a
// different order in the plan are treated as equivalent (set semantics) and
// the plan is normalized to the state order.
