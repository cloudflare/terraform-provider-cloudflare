...
// - hostnames: Set → *[]types.String (List)
// - requested_validity: Int64 → Float64 (with default 5475 if null)
...
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading origin_ca_certificate state from v4 SDKv2 provider (schema_version=0)")

	// Parse v4 state using v4 model
	var v4State SourceCloudflareOriginCACertificateModel
...
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading origin_ca_certificate state from version=1 to version=500 (no-op)")
