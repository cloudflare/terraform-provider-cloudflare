...
//
// This performs a full transformation from v4 → v5 format for the custom profile path.
// The v4 state has schema_version=0 (SDKv2 default), and we transform it to v5 format.
//
// This handler processes legacy fallback domain resources WITH policy_id.
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading fallback domain state from v4 to custom profile (schema_version=0 → version=500)")
	...
	// Transform v4 → v5 custom profile
	v5State, diags := TransformToCustomProfile(ctx, v4State)
	...
	tflog.Info(ctx, "State upgrade from v4 to custom profile completed successfully")
}
...
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading custom profile state from version=1 to version=500 (no-op)")
