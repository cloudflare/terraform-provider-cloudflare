...
func TestMagicWANGRETunnelBGPFilterIDs(t *testing.T) {
	ctx := context.Background()
	filterID := "019fb3bf3b6a7d9f8bbf3d27d87cdf44"

	var r magic_wan_gre_tunnel.MagicWANGRETunnelResource
	...
	t.Run("sets filter IDs from config", func(t *testing.T) {
		cfg := tfsdk.Config{
			Raw: tftypes.NewValue(rootObjectType, map[string]tftypes.Value{
				"bgp": tftypes.NewValue(bgpObjectType, map[string]tftypes.Value{
					"customer_asn":     tftypes.NewValue(tftypes.Number, 65002),
		...
		var got struct {
			BGP *magic_wan_gre_tunnel.MagicWANGRETunnelBGPModel `tfsdk:"bgp"`
		}
