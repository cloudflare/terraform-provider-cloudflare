...
var _ datasource.DataSourceWithConfigure = (*UserGroupsDataSource)(nil)
...
func (d *UserGroupsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := UserGroupsResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.IAM.UserGroups.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[UserGroupsResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
