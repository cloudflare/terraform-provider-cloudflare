...
func TestMigrateWorkerVersionFromV0_WithStartupTimeMs(t *testing.T) {
	...
	stateValue := tftypes.NewValue(schemaV0.Type().TerraformType(ctx), map[string]tftypes.Value{
		...
		"cache_options":       tftypes.NewValue(schemaV0.Attributes["cache_options"].GetType().TerraformType(ctx), nil),
		"limits":              tftypes.NewValue(schemaV0.Attributes["limits"].GetType().TerraformType(ctx), nil),
		"package_dependencies": tftypes.NewValue(schemaV0.Attributes["package_dependencies"].GetType().TerraformType(ctx), nil),
		...
		"exports":                 tftypes.NewValue(schemaV0.Attributes["exports"].GetType().TerraformType(ctx), nil),
		"exports_reconciliation":  tftypes.NewValue(schemaV0.Attributes["exports_reconciliation"].GetType().TerraformType(ctx), nil),
	})
