...
func TestReorderResponseBindingsToMatchPlan(t *testing.T) {
	...
	reorderedResponse, err := worker_version.ReorderResponseBindingsToMatchPlan(response, sortedPlanBindings)
	if err != nil {
		t.Fatalf("reorder response bindings: %v", err)
	}
	if err := apijson.UnmarshalComputed(reorderedResponse, &env); err != nil {
	...
	singlePlanBinding, diags := customfield.NewObjectList(ctx, []worker_version.WorkerVersionBindingsModel{
		{Name: types.StringValue("EXPECTED"), Type: types.StringValue("kv_namespace")},
	})
	if diags.HasError() {
		t.Fatalf("create single plan binding: %v", diags)
	}
	mismatchedResponse := []byte(`{
		"result": {
			"bindings": [
				{"name":"OTHER","type":"kv_namespace"}
			]
		}
	}`)
	if _, err := worker_version.ReorderResponseBindingsToMatchPlan(mismatchedResponse, singlePlanBinding); err == nil {
		t.Error("reorder mismatched response bindings: expected error")
	} else if !strings.Contains(err.Error(), `binding name mismatch: response has "OTHER", plan has "EXPECTED"`) {
		t.Errorf("reorder mismatched response bindings error = %q", err)
	}
}
