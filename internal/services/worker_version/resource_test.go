...
func testAccCloudflareWorkerVersionConfigPlainTextImport(rnd, accountID, contentFile string) string {
...
func TestAccCloudflareWorkerVersion_ImportWithPlainTextBindings(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				// Import the resource and verify no replacement is needed
				ResourceName:      resourceName,
				ImportStateIdFunc: testAccCloudflareWorkerVersionImportStateIdFunc(resourceName, accountID),
				ImportState:       true,
				ImportStateVerify: true,
				// Only ignore modules.0.content_file as it's not stored in API
				ImportStateVerifyIgnore: []string{"modules.0.content_file", "modules.0.content_base64"},
			},
			{
				// Run a plan after import to verify no changes are detected
				PreConfig: func() {
					writeContentFile(t, `export default {fetch() {return new Response()}}`)
				},
				Config: testAccCloudflareWorkerVersionConfigPlainTextImport(rnd, accountID, contentFile),
