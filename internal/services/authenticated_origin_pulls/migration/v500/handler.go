...
// The v4 state has schema_version=0 (SDKv2 default), and we transform it to v5 format with nested config structure.
func UpgradeFromV4(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading authenticated_origin_pulls state from v4 SDKv2 provider (schema_version=0)")

	// Parse v4 state using v4 model
	var v4State SourceCloudflareAuthenticatedOriginPullsModel
...
func UpgradeFromV5(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	tflog.Info(ctx, "Upgrading authenticated_origin_pulls state from version=1 to version=500 (no-op)")
