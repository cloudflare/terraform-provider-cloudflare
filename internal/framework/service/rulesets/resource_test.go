...
func TestAccCloudflareRuleset_CacheSettingsValidBrowserTTLWithOverrideOrigin(t *testing.T) {
	rnd := utils.GenerateRandomResourceName()
	zoneID := os.Getenv("CLOUDFLARE_ZONE_ID")
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCloudflareRulesetCacheSettingsValidDefaultBrowserTTLOverrideOrigin(rnd, zoneID),
			},
		},
	})
}
...
func testAccCloudflareRulesetCacheSettingsValidDefaultBrowserTTLOverrideOrigin(rnd, zoneID string) string {
	return fmt.Sprintf(`
  resource "cloudflare_ruleset" "%[1]s" {
	zone_id     = "%[2]s"
    name        = "%[1]s"
