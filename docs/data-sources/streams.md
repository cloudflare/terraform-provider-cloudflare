...
- `after` (String) Alias for 'start'. Returns videos created after this date/time (RFC 3339 format).
- `asc` (Boolean) Lists videos in ascending order of creation.
- `before` (String) Alias for 'end'. Returns videos created before this date/time (RFC 3339 format).
- `creator` (String) A user-defined identifier for the media creator.
- `end` (String) Lists videos created before the specified date.
- `id` (String) Filter by video ID(s). Can be a single ID or a comma-separated list of IDs.
...
- `public_details` (Attributes) Public details for the video including title, share link, channel link, and logo. (see [below for nested schema](#nestedatt--result--public_details))
...
<a id="nestedatt--result--public_details"></a>
### Nested Schema for `result.public_details`
