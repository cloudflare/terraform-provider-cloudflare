...
- `content_type` (String) Content type of the file, such as `application/wasm`, `text/plain`, or `application/octet-stream`.

Optional:

- `content_base64` (String) Base64-encoded file content.
- `content_file` (String) Path to the file content.

Read-Only:

- `content_sha256` (String) SHA-256 hash of the file content, used to detect changes and remote drift.
